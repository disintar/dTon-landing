...
export const ConnectAndTry = ({style,size='small'}) => {
    ...
    return <button style={{ width: 120,
        ...
        onClick={()=> window.open('https://tech.dton.co/profile','_blank')}>
...
export const Documentation = ({style = {},size = 'small'}) => {
    ...
    return <button
            ...
            onClick={() => window.open('https://docs.dton.co/', '_blank')}
            className='btn btn-primary outline'>
        Documentation
    </button>
}
...
export const EmailInput = () => {
    ...
    const handleEmail = () => {
        if(validateEmail(email)){
            axios.post(`https://status.dton.co/api/v1/email_subscribe/${email}/`).then(({data}) => {
                if(data?.success){
