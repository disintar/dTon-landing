    ...
    return <Row style={blockStyle}>
                 <Col md={6} sm={12} span={12}>
                    <Flex gap={5} vertical align="center">
                       {day ? <Typography.Title style={valueStyle} level={2}>{percentToString(day)}</Typography.Title> : <Spinner />}
                        <Typography.Text style={textStyle}>Uptime / 24h</Typography.Text>
                    </Flex>
                </Col>
                <Col md={6} sm={12} span={12}>
                    <Flex gap={5} vertical align="center">
                        {week ? <Typography.Title style={valueStyle} level={2}>{percentToString(week)}</Typography.Title> : <Spinner />}
                        <Typography.Text style={textStyle}>Uptime / 7d</Typography.Text>
                    </Flex>
                </Col>
                <Col md={6} sm={12} span={12}>
                    <Flex gap={5}  vertical align="center">
                       {call_latency ? <Typography.Title style={valueStyle} level={2}>{callLatencyToString(call_latency)}</Typography.Title> : <Spinner />}
                        <Typography.Text style={textStyle}>Call Latency</Typography.Text>
                    </Flex>
                </Col>
                <Col md={6} sm={12} span={12}>
                    <Flex gap={5} vertical align="center">
                      {index_latency ? <Typography.Title style={valueStyle} level={2}>{indexLatencyToString(index_latency)}</Typography.Title> : <Spinner />}
